...
export default function CertificateList({ itemList, skillList }: Props) {
  ...
  return (
    <div className="flex flex-row">
      <div className="grid w-3/4 grid-cols-3 gap-1 mt-10 ml-5 h-fit">
        {list?.map((item) => {
          return (
            <Card key={item.name} styling="flex flex-row p-2 h-20 m-1 bg-white">
              <div className="flex flex-col">
                <div className="font-bold">{item.name}</div>
                <div className="w-full m-1 ml-2 text-xs font-semibold text-gray-500">
              ...
              <LinkButton
                href={`/my-certs/${item.name}`}
                styling="w-fit ml-auto ml-2"
              >
                <svg
      ...
      <div className="w-1/4 mt-10 ml-5">
        <Card styling="mr-6 p-6 bg-white">
          <ul className="flex flex-row flex-wrap">
            <CertificateButton
              skill={locale === "en-US" ? "All" : "Todos"}
